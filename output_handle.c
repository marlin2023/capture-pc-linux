...
void encode_audio_frame(Output_Context *ptr_output_ctx1 , uint8_t *buf ,int buf_size ){

	Output_Context *ptr_output_ctx = ptr_output_ctx1;
	int ret;
...
//	av_write_frame(ptr_output_ctx->ptr_format_ctx, &pkt);
	...
	//printf("prog_num = %d , pkt.stream = %d \n" ,prog_num ,pkt.stream_index);
		pthread_mutex_lock(&(ptr_output_ctx1->output_mutex));
//		av_write_frame(ptr_output_ctx1[i]->ptr_format_ctx, &pkt);
		av_interleaved_write_frame(ptr_output_ctx1->ptr_format_ctx, &pkt);
		pthread_mutex_unlock(&(ptr_output_ctx1->output_mutex));
...
void do_audio_out(Output_Context *ptr_output_ctx1 ,AVFrame *decoded_frame ,int nb_sample){
...
//	int size = src_audio_buf_size;
	uint8_t *buf = (uint8_t * )decoded_frame->data[0];
	int size = decoded_frame->nb_samples
					* 2		//dec_channels
					* isize;
...
//			printf("av_fifo_size(ost->fifo) = %d ,frame_bytes = %d\n" ,av_fifo_size(ptr_output_ctx->fifo) ,frame_bytes);
			...
			encode_audio_frame(ptr_output_ctx1, ptr_output_ctx->audio_buf, frame_bytes);
			//printf("encode audio ....\n");
		}
